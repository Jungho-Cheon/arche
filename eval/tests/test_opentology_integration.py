"""통합: Opentology 컬럼 ↔ apps/api FastAPI app (FakeGraph 어댑터) e2e.

FastAPI 코어를 httpx.ASGITransport 로 묶어 OpentologyClient 가 *실제 HTTP 계약*
...
pytest.importorskip("opentology_api", reason="apps/api 가 같은 venv 에 없으면 integration 스킵")

from opentology_api.adapters.embedding import EmbeddingProvider  # noqa: E402
from opentology_api.adapters.graph import (  # noqa: E402
    EntityTypeStat,
    EntityWithCounts,
    GraphRepository,
    KeywordHit,
...
from opentology_api.adapters.llm import LLMProvider as CoreLLMProvider  # noqa: E402
from opentology_api.api.deps import (  # noqa: E402
    embedding_provider_dep,
    graph_repo_dep,
    llm_provider_dep,
)
from opentology_api.domain.models import (  # noqa: E402
    Edge,
    ExtractedGraph,
    Node,
    SourceRef,
...
from opentology_api.main import create_app  # noqa: E402
...
def httpx_client_against_api() -> httpx.Client:
    """eval 의 OpentologyClient 에 주입할 sync httpx.Client.

    WHY MockTransport + TestClient 위임: httpx.ASGITransport 는 async-only 라
    OpentologyClient (sync) 가 직접 호출하지 못한다. 대신 sync FastAPI TestClient
    를 띄우고, MockTransport 의 handler 에서 TestClient 호출로 위임한다.
    이렇게 하면 OpentologyClient 의 sync HTTP 코드 경로는 그대로 실행되면서
...
def test_one_question_e2e_through_fastapi(httpx_client_against_api: httpx.Client) -> None:
    ...
    client = OpentologyClient(base_url="http://test", client=httpx_client_against_api)
    ...
    runner = OpentologyRunner(client=client, answer_llm=llm)
