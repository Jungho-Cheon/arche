"""Opentology 컬럼 단위 — provider + client 모두 mock 처리.
...
from opentology_eval.columns.opentology import (
    OpentologyRunner,
    _decide_combination,
...
def _mock_opentology_client(handlers: dict[str, Any]) -> OpentologyClient:
    ...
    return OpentologyClient(base_url="http://test", client=inner)
...
def test_one_entry_point_calls_subgraph_only() -> None:
    ...
    client = _mock_opentology_client(handlers)
...
def test_payload_has_all_prd_36_required_keys() -> None:
    ...
    assert payload["column"] == "opentology"
