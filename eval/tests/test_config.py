...
from opentology_eval.config import (
...
def test_default_models_pin_control_variables(monkeypatch) -> None:
    monkeypatch.delenv("OPENTOLOGY_EVAL_LLM_MODEL", raising=False)
    monkeypatch.delenv("OPENTOLOGY_EVAL_EMBEDDING_MODEL", raising=False)
    cfg = load_config()
...
def test_env_overrides(monkeypatch) -> None:
    monkeypatch.setenv("OPENTOLOGY_EVAL_LLM_MODEL", "openai/gpt-4o-mini")
    monkeypatch.setenv(
        "OPENTOLOGY_EVAL_EMBEDDING_MODEL", "openai/text-embedding-3-large"
    )
    cfg = load_config()
...
def test_provider_prefix_normalized_when_omitted(monkeypatch) -> None:
    ...
    monkeypatch.setenv("OPENTOLOGY_EVAL_LLM_MODEL", "gpt-4.1")
    monkeypatch.setenv("OPENTOLOGY_EVAL_EMBEDDING_MODEL", "text-embedding-3-small")
    cfg = load_config()
