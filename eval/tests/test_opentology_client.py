"""Opentology REST 클라이언트 단위 — httpx.MockTransport 로 응답 mock."""
...
from opentology_eval.clients import (
    OpentologyClient,
...
def _mock_client(handler) -> OpentologyClient:
...
def test_error_envelope_raises_client_error() -> None:
    ...
    client = _mock_client(handler)
    with pytest.raises(OpentologyClientError) as ei:
...
def test_network_failure_raises_unavailable() -> None:
    ...
    client = _mock_client(handler)
    with pytest.raises(OpentologyUnavailableError):
        client.get_schema()
...
def test_primitive_call_log_records_error_when_envelope_returns_error() -> None:
    ...
    client = _mock_client(handler)
    log: list = []
    with pytest.raises(OpentologyClientError):
