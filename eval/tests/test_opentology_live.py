"""End-to-end live: 실제 Neo4j + OpenAI 위에서 Opentology 컬럼 1 질문 흐름.

활성화 조건:
  RUN_LIVE_TESTS=1
  OPENTOLOGY_API_URL  (코어가 떠 있는 base URL)
  OPENAI_API_KEY      (.env 또는 환경 변수)

코어 (apps/api) 는 본 테스트 전에 *외부에서* 부팅돼 있어야 한다 — 본 테스트는
구동 책임을 지지 않는다. 사용 예:
  $ docker compose up -d neo4j api
  $ OPENTOLOGY_API_URL=http://localhost:8000 RUN_LIVE_TESTS=1 uv run pytest tests/test_opentology_live.py -v
"""
...
def _live_or_skip() -> tuple[str, str]:
    ...
    api_url = os.environ.get("OPENTOLOGY_API_URL")
    if not api_url:
        pytest.skip("OPENTOLOGY_API_URL not set")
    try:
...
def test_opentology_live_one_question() -> None:
