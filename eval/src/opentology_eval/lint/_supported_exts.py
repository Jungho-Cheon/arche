...
WHY 복제 (직접 import 대신):
  - PRD 4 §0 — eval 패키지는 코어 (`apps/api`) 와 격리되어야 한다. eval 의
    `opentology` 컬럼이 코어를 HTTP 로만 호출하는 것과 동일한 경계.
  - 본 상수만 한 줄 import 하기 위해 `opentology-api` 전체를 eval 의 의존성에
    추가하면 격리가 깨진다 (코어의 모든 모듈을 eval 이 끌고 들어옴).
  - 그래서 *상수만 복제* 하고 동기화 의무를 코멘트에 명시.

PRD 2 §2.1 (`apps/api/src/opentology_api/domain/ingest.py`) 의
