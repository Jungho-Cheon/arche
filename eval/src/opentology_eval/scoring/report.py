...
| 조건 | 비고 |
|---|---|
| Opentology accuracy >= full_context accuracy | 정확도 보존 |
| Opentology total_tokens median <= chunk_rag total_tokens median | 토큰 효율 |
| Opentology latency median <= chunk_rag latency median | 지연 효율 |
...
def evaluate_pareto(
    *,
    ...
    opentology: ColumnMetrics,
) -> ParetoVerdict:
    """본 PR 의 비엄격 부등식 판정.

    - accuracy: Opentology >= Full-context
    - tokens:   Opentology median <= Chunk RAG median
    - latency:  Opentology median <= Chunk RAG median
    """
    reasons: list[str] = []

    accuracy_ok = opentology.accuracy + 1e-9 >= full_context.accuracy
    if not accuracy_ok:
        reasons.append(
            f"Accuracy {opentology.accuracy:.3f} < Full-context {full_context.accuracy:.3f}"
        )

    tokens_ok = opentology.total_tokens_median <= chunk_rag.total_tokens_median + 1e-9
    if not tokens_ok:
        reasons.append(
            f"Total tokens (median) {opentology.total_tokens_median:.0f} > "
            f"Chunk RAG {chunk_rag.total_tokens_median:.0f}"
        )

    latency_ok = opentology.latency_ms_median <= chunk_rag.latency_ms_median + 1e-9
    if not latency_ok:
        reasons.append(
            f"Latency (median) {opentology.latency_ms_median:.0f}ms > "
...
def render_markdown(agg: RunAggregate, *, run_ts: str = "") -> str:
    ...
    op = agg.columns.get("opentology")

    verdict: ParetoVerdict | None = None
    if fc and cr and op:
        verdict = evaluate_pareto(full_context=fc, chunk_rag=cr, opentology=op)

    lines: list[str] = []
    lines.append(f"# Opentology MVP 측정 보고서 — {run_ts}".rstrip())
    lines.append("")
    lines.append(
...
def _render_paragraph(agg: RunAggregate, verdict: ParetoVerdict | None) -> str:
    ...
    return (
        ...
        f"Opentology {_fmt_pct(op.accuracy)} 다. "
...
def write_report(
    ...
    *,
    ...
    pareto = None
    if fc and cr and op:
        v = evaluate_pareto(full_context=fc, chunk_rag=cr, opentology=op)
        pareto = {
            "passed": v.passed,
