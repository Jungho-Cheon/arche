"""컬럼 (4) Combined — chunk RAG 발췌 + Opentology 서브그래프를 단일 LLM 호출로 합침.
...
격리 / 통제 변수:
  ...
  - anchor / find_entities / get_subgraph / find_path 흐름은 OpentologyRunner
...
class CombinedRunner:
    """chunk RAG retrieval + opentology subgraph 를 단일 호출에 묶음.
    ...
    def setup(self) -> None:
        """chunk RAG 인덱스 구축. (opentology setup 은 별도 ingest 로 미리 끝나 있다 가정)"""
    ...
    def _extract_anchors(self, question_text: str) -> _AnchorResult:
        """OpentologyRunner._extract_anchors 와 동일 동작 (1 회 재시도)."""
