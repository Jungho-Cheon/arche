...
WHY: PRD 4 §2.7 통제 변수 — 컬럼 (1)(2)(3) 모두 *같은 LLM*, 청크 RAG 와 Opentology
...
# WHY text-embedding-3-small: 컬럼 (2) chunk RAG 와 (3) Opentology 노드 임베딩의
...
def load_config() -> EvalConfig:
    ...
    return EvalConfig(
        llm_model=_normalize(
            os.environ.get("OPENTOLOGY_EVAL_LLM_MODEL", DEFAULT_LLM_MODEL)
        ),
        embedding_model=_normalize(
            os.environ.get("OPENTOLOGY_EVAL_EMBEDDING_MODEL", DEFAULT_EMBEDDING_MODEL)
        ),
