...
본 모듈이 한 곳에 모아 두는 이유:
  세 컬럼 (`full_context` / `chunk_rag` / `arche`) 의 응답 페이로드 스키마가
  *서로 다르다* . `full_context` 는 `input_tokens / output_tokens / latency_ms` 만,
  `chunk_rag` 는 `embedding_tokens / total_tokens` 도 추가, `arche` 는 anchor
...
COLUMNS: tuple[str, ...] = ("full_context", "chunk_rag", "arche", "combined")
...
class ResponseMetrics:
    ...
    Fields:
        ...
        embedding_tokens: 임베딩 토큰 (chunk_rag 의 question + amortized setup,
                          arche 의 keyword 추정 등). 없으면 0.
        total_tokens:  input + output + embedding 합. 토큰 메트릭의 단일 진실의 원천.
        latency_ms:    응답 전체 wall-clock 지연. arche 는 anchor + primitives + answer 합.
...
def extract_metrics(response: dict[str, Any], column: str) -> ResponseMetrics:
    ...
    if column == "full_context":
        ...
        # combined 응답 스키마는 arche 와 동일 구조 — anchor + answer_generation
