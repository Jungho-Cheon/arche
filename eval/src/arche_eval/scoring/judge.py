...
def build_mapping(
    ...
    columns: tuple[str, ...] = ("full_context", "chunk_rag", "arche"),
    *,
    seed: int | None = None,
...
def invert_mapping(mapping_for_question: dict[str, str]) -> dict[str, str]:
    """{A: full_context, B: chunk_rag, C: arche} → {full_context: A, ...}."""
...
def extract_student_reasoning(response: dict[str, Any]) -> str:
    ...
    Arche 컬럼은 `answer_generation.parsed.reasoning` 에 위치 (full_context /
