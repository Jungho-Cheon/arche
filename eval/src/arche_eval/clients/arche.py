"""Arche 코어 REST 클라이언트 — PRD 3 의 6 primitive + admin ingest.

격리 원칙 (ADR-0006 D4) — 본 컬럼은 코어를 *외부 시스템* 으로만 호출한다.
`arche_api` 를 직접 import 하지 않는다. 테스트의 FastAPI TestClient 경유는
...
class ArcheClientError(RuntimeError):
...
class ArcheUnavailableError(RuntimeError):
...
class ArcheClient:
    ...
    def __enter__(self) -> "ArcheClient":
        return self
    ...
    def _request(
        ...
        에러 envelope → ArcheClientError. 네트워크 / 비-JSON → ArcheUnavailableError.
        """
        t0 = time.perf_counter()
        try:
            resp = self._client.request(method, path, json=json)
        except (httpx.TimeoutException, httpx.NetworkError, httpx.TransportError) as e:
            raise ArcheUnavailableError(
    ...
    def _call_primitive(
        ...
        *,
        ...
        try:
            ...
            raise
        except ArcheUnavailableError as e:
            if log is not None:
                log.append(
                    PrimitiveCall(
                        name=name,
    ...
    def wait_for_ingest(
        ...
        *,
        ...
        실패 시 ArcheClientError raise (코어 응답의 error.code/message 보존).
