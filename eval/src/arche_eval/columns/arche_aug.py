"""컬럼 (5) Arche Augmented — graph-guided chunk retrieval (PoC).
...
격리 / 통제 변수:
  - chunking / embedding / TOP_K / hops / max_nodes 모두 chunk_rag /
    arche / combined 와 동일
  - 답변 시스템 프롬프트는 본 PoC 의 별도 한 줄 (ARCHE_AUG_SYSTEM)
"""
...
ARCHE_AUG_SYSTEM = """당신은 도메인 전문가입니다. 아래에 같은 코퍼스에서 결합된
두 정보가 제공됩니다:

  (A) 그래프 진입점이 가리킨 *원본 발췌* — 질문에서 추출한 도메인 엔티티가
      어느 문서와 청크에서 비롯됐는지 추적해, 그 문서들 안에서 질문 임베딩으로
...
class ArcheAugRunner:
    ...
    Attributes:
        ...
        그 외: ArcheRunner / CombinedRunner 와 동일 디폴트.
    """
    ...
    def setup(self) -> None:
        ...
        arche 코어 ingest 는 별도 절차 (admin_ingest) — 이미 완료됐다 가정.
    ...
    def ask(
        ...
        *,
        ...
        # (1) anchor 추출 — arche / combined 와 동일
    ...
    def _extract_anchors(self, question_text: str) -> _AnchorResult:
        """ArcheRunner._extract_anchors 와 동일."""
