"""프롬프트 — PRD 4 §1.3-1.4 (full-context), §2.5-2.6 (chunk RAG), §3.2/§3.4 (arche) 의 한국어 본문 그대로.
...
# --- Arche 컬럼 (PRD 4 §3.2, §3.4) — anchor 추출 + 답변 생성 ---
...
2. 질문이 "어떻게 처리되나" / "어떻게 적용되나" / "어떻게 해소되나" 형태이면, 답에 필요한
   ...
   함께 추출. 흐름의 *이름* 까지만.
3. 파생 지표와 계산 지표의 구성 항목 분해 (도메인 무관) — 질문이 *직접 저장되어 있지 않고
   ...
   재고; "gross margin" → 매출과 매출원가; "YoY 성장률" → 해당 지표의 각 기간 값.
   여기서는 *구성 항목의 이름* 을 키워드로 추출하는 것이며 (구체적 숫자값을 지어내는
   것이 아님), 이름이 질문 대상 (회사나 실험 등) 과 함께 검색되도록 한다.
...
# 기존 원칙 "추측과 확장 금지" 가 그래프 단독 답변을 과도하게 보수화시켜, 관련 엔티티를
# 실제로 검색해 놓고도 33문항 중 26번을 "정보 부족" 으로 회피했다 (graphify 그래프
# 단독 14/33 회피, arche 21.2% vs graphify 57.6%). 진단: 그래프는 *관계를 연결해
# 결론에 이르라고* 주는 것인데, "BCA(상업용 항공기) 부문" 노드가 있어도 "상업 항공
# 수요에 노출" 이라는 상식 추론을 "확장" 으로 보고 거부한 것이 원인. 따라서 (1) 그래프에
# 등장한 엔티티와 관계로부터의 정당한 추론은 허용하되, (2) 그래프에 없는 사실/수치 날조는
# 계속 금지하도록 균형을 다시 잡는다. 숫자 환각으로 정답률이 부풀려지지 않게 마지막
# 원칙으로 수치 사용을 그래프 내 실재 값으로 한정.
ARCHE_ANSWER_SYSTEM = """당신은 도메인 전문가입니다. 아래에 그래프 형태로 추출된 도메인 지식(엔티티와 관계)을 읽고,
사용자의 질문에 대한 정답 보기를 고른 뒤 이유를 설명하세요.
...
- "정보 부족" 은 답에 필요한 엔티티와 관계 자체가 그래프에 없을 때만 고른다. 관련 엔티티가 그래프에 있는데 단지 명시적 서술 문장이 없다는 이유로 "정보 부족" 을 고르지 않는다.
- 구체적 수치(재무 비율과 금액 등)는 그래프의 노드/속성에 그 값이 실제로 있을 때만 사용한다. 그래프에 없는 수치를 추정해 보기를 고르지 않는다 (이때는 "정보 부족").
