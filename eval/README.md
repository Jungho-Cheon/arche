# Opentology MVP — 평가 하니스 (베이스라인 두 컬럼)

본 패키지는 Opentology MVP 의 3-way 비교 측정 중 **컬럼 (1) full-context LLM** 과 **컬럼 (2) chunk 벡터 RAG** 를 구현한다. 컬럼 (3) Opentology 와 judge / spotcheck / report 는 후속 이슈 (#10, #11) 에서 다룬다.
...
- **OpenAI text-embedding-3-small (1536-dim)** — 컬럼 (2) chunk RAG 와 향후 컬럼 (3) Opentology 노드 임베딩의 *통제 변수*. 가격 대비 품질이 충분.
