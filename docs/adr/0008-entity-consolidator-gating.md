...
ADR-0007 (Combined RAG 채택) 의 D2 가 정의한 1M 시점 재검증 (M6.5) 을 2026-06-20 FinanceBench 1M corpus (980K tokens, 33 MCQ) 에 실행했다. 결과 **chunk_rag 72.7% / arche 6.1% / combined 72.7%** — Combined 가 chunk 와 같은 정확도. 표면적으로 ADR-0007 D2 의 "Combined ≈ chunk → D6 만 살리고 M7 단순화" 분기로 들어가야 하지만, *진단 결과* graph 데이터가 **catastrophic over-merge** 로 부패해 측정 자체가 무효.
...
3. **M6.5b 종료 후 arche + combined 만 재측정**해 ADR-0007 D2 의 진짜 분기를 결정한다.
...
| arche (graph 단독) | 6.1% | 4.1K | 3.56s | $1.20 |
| combined (chunk + graph) | 72.7% | 41.1K | 4.90s | $2.66 |

arche 는 33 문항 중 31 개에 "정보 부족 (e)" 응답.

### 1 차 가설과 그 기각

처음에는 anchor extraction 의 *언어 mismatch* 가 원인이라 가설했다 — 영어 corpus 에 대해 anchor 가 한국어 entity ("당좌비율", "재무분석가") 를 만들어 그래프 매칭이 실패. 이는 사실이었고, `eval/src/arche_eval/prompts.py:65` 의 ANCHOR_EXTRACTION_SYSTEM 프롬프트에 *질문의 언어를 보존* 하라는 원칙 + 영어 예시를 추가해 수정했다.

수정 후 anchor 는 영어로 정상 추출. **그러나 arche 정확도는 6.1% 그대로**.
...
2. 다음 10-K (AMD_2022) 에서 동일 텍스트 "the Company" 가 *embedding cosine ≥ 0.92* (`apps/api/src/arche_api/domain/identity.py` 의 `EMBEDDING_MATCH_THRESHOLD`) 를 만족 → 기존 Amcor 의 "the Company" entity 에 merge → AMD 도 alias 로 흡수
...
- arche + combined 만 N=3 재측정. chunk_rag 는 본 회차 결과 (`eval/runs/2026-06-20-1426/responses/chunk_rag/`) 그대로 재사용
...
- `eval/runs/2026-06-20-1426/responses/{chunk_rag,arche,combined}/` (raw) — main 보존
...
- **EntityMatcher 의 4 단계 + cosine ≥ 0.92 임계값은 *cross-document generic 자기지칭에 취약*** — `apps/api/src/arche_api/domain/identity.py` 수정 시 본 ADR 을 참조 (특히 EMBEDDING_MATCH_THRESHOLD)
