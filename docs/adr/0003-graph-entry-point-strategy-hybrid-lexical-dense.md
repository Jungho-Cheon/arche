...
- Arche 의 (3) 컬럼 — 그래프 노드 RAG + 탐색 — 이 *질문 → 진입점* 단계를 어떻게 푸는지 알고 싶다면
- "왜 dense 임베딩 만 쓰지 않는가" 또는 "왜 BM25 만 쓰지 않는가" 가 궁금하다면
- 본인의 도메인이 상거래가 아닐 때 (생물학이나 연구 문헌 등) 진입점 전략이 어떻게 바뀔 여지가 있는지
...
지금까지 RAG 의 디폴트. 동의어와 의역에 강하다 ("주문 취소" 와 "결제 환불"). 단점은 (1) 식별자에 약함, (2) 임베딩 인프라가 무거움, (3) *식별자가 본문에 그대로 등장하는* 도메인에서는 BM25 보다 떨어질 수 있음.
...
### D1. 진입점 선정 = (caller 의 anchor 추출) + (Arche 의 하이브리드 매칭)

질문 → 진입점 노드의 흐름을 두 책임으로 분리한다.

**caller 책임** (Arche 의 컴포넌트가 아님):

1. **LLM anchor 추출** — caller (에이전트 또는 벤치마크 하니스) 가 자체 LLM 사이클로 질문에서 *언급된 엔티티의 정규명 + 가능한 별칭* 을 추출한다. 권장은 LLM 에 정규명과 별칭을 JSON 으로 함께 반환하라고 시키는 것. 자세한 분리 근거는 ADR-0006 참조.

**Arche 책임** (`find_entities(keywords, ...)` primitive 안에서 일어남):

2. **어휘 매칭** — caller 가 보낸 키워드 (anchor 정규명과 별칭) 를 그래프 내 엔티티 이름과 별칭에 *정확 / fuzzy 문자열 매칭* (BM25 류). 식별자 중심 도메인 (상거래) 에서 강한 신호.
3. **dense 임베딩 유사도** — Arche 가 받은 키워드를 *server-side 에서 임베딩* 한 뒤, 노드 단위로 저장된 임베딩과의 유사도를 ANN 인덱스로 조회. 어휘가 놓치는 동의어/의역 케이스를 흡수.
...
- **검증 가설 (ADR-0001 D2) 의 통제 변수와 정합** — 비교 대상인 청크 벡터 RAG 가 *청크 임베딩* 을 쓰고, Arche 가 *노드 임베딩* 을 쓴다. 정확도 차이의 원인이 *retrieval 단위* 에서 비롯됨을 보장.

### D3. 별칭 정규화는 ingest 단계 (Arche) + query 단계 (caller) 둘 다

별칭 처리는 두 시점에 들어가는데, *책임 위치가 다르다* .

- **Ingest 시 (Arche 책임)** — LLM 이 소스에서 엔티티를 추출할 때 *정규명 + 본문에 등장하는 변형 표현* 을 별칭 속성으로 함께 저장. 그래프 자체가 별칭 사전을 가진다.
...
또한 ADR-0001 의 검증 통제 변수 — *동일 임베딩 모델* 이 청크 RAG 와 Arche 양쪽에 적용 — 가 무너진다. Arche 가 임베딩을 쓰지 않으면 두 시스템의 인프라가 *너무 달라져* 비교의 통제가 어려워진다.
...
- Arche 코어는 *ingest 시점에만 LLM 을 호출* . query 시점 LLM 호출은 caller (ADR-0006). 코어의 외부 의존성이 줄어듦.
- query 시 Arche 가 *embedding 모델은 호출* . caller 가 보낸 키워드를 server-side 에서 임베딩하기 위함. (LLM 호출과 embedding 호출은 다르다 — embedding 은 훨씬 싸고 빠르며 동일성도 강함.)
...
- `find_entities` primitive 가 받는 `keywords` 인자는 *이미 caller 가 추출한 anchor* 라고 가정. Arche 가 *그 키워드에서 다시 LLM 으로 별칭을 추출하지 않음* (분리 원칙).
