...
Opentology 는 MCP 와 REST API 모두에서 **graph primitives** — `find_entities`, `get_entity`, `get_neighbors`, `find_path`, `get_subgraph`, `get_schema` — 만 노출한다. *자연어 질문 엔드포인트는 두지 않는다.* caller (에이전트 또는 벤치마크 하니스) 가 자체 LLM 사이클로 질문에서 anchor 키워드를 추출한 뒤 primitives 를 호출한다. 이 분리는 (1) MCP 의 통상 패턴과 정렬, (2) Opentology 코어에서 쿼리 시점 LLM 의존성 제거, (3) Pareto 우월 가설 (ADR-0001) 의 토큰 회계를 caller 쪽으로 정직하게 이전 — 세 효과가 있다.
...
- Opentology 가 *어떤 표면* 으로 외부와 만나는지 알고 싶다면
- "왜 `/query` 같은 자연어 엔드포인트가 없는가" 가 궁금하다면
- Neo4j MCP 가 이미 존재하는데 *왜 Opentology MCP 를 따로 만드는지*
- post-MVP 에서 채팅 기능이 들어올 때 *Opentology 의 표면이 어떻게 진화할지*
...
ADR-0003 의 초기 안은 *"Opentology 가 자연어 질문을 받아 내부에서 LLM 으로 anchor 를 추출"* 하는 모델이었다. 이는 두 가지 부담을 만든다.

1. **쿼리 시점 LLM 컴포넌트** — Opentology 코어가 쿼리마다 LLM API 를 호출. 추가 의존성·비용·지연.
2. **에이전트와의 중복** — 에이전트는 어차피 자체 LLM 사이클로 *어떤 정보가 필요한지* 를 정한다. 그 결정을 자연어 문장으로 다시 묶어 Opentology 에 넘기면, Opentology 가 *같은 LLM 사이클을 한 번 더 돈다* . 이중 작업.
...
1. **에이전트 부담** — 에이전트가 *Cypher 문법 + Opentology 의 정확한 스키마 + 하이브리드 매칭 (어휘 + 벡터) 작성법* 을 모두 알아야 한다. `find_entities(["쿠폰"])` 한 줄로 가능한 일을 *full-text index 호출 + vector index 호출 + RRF 결합* Cypher 로 표현하라고 시키는 셈.
...
Opentology 가 자체 MCP 를 가져야 하는 이유는 *Neo4j MCP 의 대체* 가 아니라 *그 위의 semantic abstraction layer* 가 필요하기 때문이다. 두 MCP 는 *같은 그래프 DB 인스턴스* 에 동시에 붙을 수 있어 (Neo4j MCP = power user / 디버깅용, Opentology MCP = 에이전트의 일상 사용) *경쟁이 아니라 보완* 관계다.
...
| Primitive | 입력 | 출력 | 용도 |
|---|---|---|---|
| `get_schema()` | (없음) | 엔티티 타입 목록 + 관계 타입 목록 + 타입별 샘플 N 개 | 에이전트가 그래프의 *모양* 을 파악. Neo4j MCP `get-schema` 와 유사한 역할이지만 Opentology 의미론 (엔티티/관계/소스) 로 노출 |
...
- **CLI / admin REST 엔드포인트** — `opentology ingest <directory>` 같은 명령. 본인 (운영자) 만 사용.
...
벤치마크 하니스 (ADR-0001 의 (3) 컬럼 측정) 도 같은 흐름. 즉 **anchor 추출과 답변 생성 두 LLM 호출은 caller 쪽에서 발생** 하고, Opentology 는 그 사이의 retrieval 만 책임진다.

토큰 회계 (ADR-0005 D6) 는 *system 전체* 의 토큰을 계산하므로 caller 의 두 LLM 호출도 *Opentology 컬럼* 의 토큰으로 합산된다. 즉 *책임 위치만 바뀌고 토큰 합계는 동일* . Pareto 우월 가설 검증에는 영향 없음.

### D5. Post-MVP — 자연어 wrapper 의 진화 경로

MVP 종료 후 *외부 사용자* (본인 외) 가 등장하거나 *Opentology 자체가 채팅 기능* 을 제공하게 되면 자연어 입력 수요가 생긴다. 그때의 진화 경로는 **얇은 wrapper 추가** 다.

```
[자연어 질문] → opentology 의 chat layer (별도 컴포넌트) →
  LLM 으로 anchor 추출 → 내부적으로 primitives 호출 → 결과 →
  LLM 으로 답변 생성 → [자연어 답변]
```

핵심은 **chat layer 가 Opentology 코어와 분리된다** 는 것. 코어는 여전히 graph primitives 만 노출. wrapper 는 *post-MVP 의 별도 컴포넌트* 로 정당화되고 (그때는 채팅이 product surface 이므로), 코어에 LLM 의존성을 끌어들이지 않는다.

이 분리가 *지금 결정* 의 가장 큰 효용 — 미래에 추가될 chat layer 가 *코어의 변경 없이 위에 얹히는* 형태로 진화한다.

### D6. Neo4j MCP 와의 공존 가이드

Opentology MCP 와 Neo4j MCP 는 *같은 DB 인스턴스에 동시 attach 가능* . 권장 사용 분리:

| 시나리오 | 권장 도구 |
|---|---|
| 에이전트가 도메인 답을 만들기 위해 그래프를 조회 | **Opentology MCP** (semantic primitives) |
| 그래프 schema 디버깅, ad-hoc 분석, GDS 알고리즘 실행 | **Neo4j MCP** (raw Cypher) |
| 데이터 백업/관리/마이그레이션 | Neo4j MCP 또는 직접 Cypher |
| 에이전트의 일상 read 작업 | Opentology MCP (안전, 추상화) |
...
거부. **(a) 에이전트 워크플로와 어긋난다 + (b) 코어에 query-time LLM 의존성을 끌어들인다 + (c) 토큰 이중 계산이 발생한다** . 에이전트는 자체 LLM 으로 *반복적인 탐색 결정* (한 번 검색 → 결과 보고 다음 검색) 을 하는데, 자연어 입출력만 노출되면 그 *결정 사이클이 Opentology 의 자연어 처리로 한 번 더 wrap 됨* . 토큰·지연이 누적.

만약 이걸 택했다면, Opentology 의 컴포넌트 다이어그램에 *쿼리 시점 LLM* 박스가 영구히 들어왔을 것이고, post-MVP 의 chat 기능이 *이 LLM 박스를 재사용할지 새로 둘지* 가 또 고민거리가 됐을 것이다.
...
### 옵션 3 — Neo4j MCP 를 그대로 쓰고 Opentology MCP 안 만들기

거부. *에이전트의 부담이 너무 크고, 안전성이 떨어진다.* Cypher 작성을 에이전트에게 떠넘기면 (a) *Opentology 의 정확한 스키마* (엔티티 타입·관계 타입·속성 명) 를 매번 LLM 컨텍스트에 넣어야 하고, (b) 하이브리드 매칭 (어휘 + 벡터) 같은 *Opentology 의 고유 의미론* 을 Cypher 로 표현하라고 시켜야 한다. 또한 write Cypher 노출은 *에이전트가 그래프를 부술 수 있는* 시나리오를 만든다.
...
거부. *에이전트에게 결합 로직을 떠넘기는 셈* . 하이브리드 매칭은 *Opentology 의 retrieval 결정* (ADR-0003) 이므로 *한 primitive 안에서 결정* 되는 게 맞다. 에이전트가 *어휘 점수와 벡터 점수를 받아 RRF 로 결합* 하는 건 *재발명* 이다. 단, 정말 *분리된 신호가 필요한* 경우를 위해 `find_entities` 의 응답에 *각 신호의 raw 점수* 를 부록으로 포함시킬 수는 있다 (구현 단계 결정).

## Consequences

### 즉시 영향

- **ADR-0003 D1 step 1** (LLM anchor 추출) 의 책임 주체가 *Opentology → caller* 로 이전됨. ADR-0003 본문 갱신 필요.
- **ADR-0001 D3 (3) Opentology flow** 가 *caller 의 두 LLM 호출 + Opentology 의 primitives 호출* 로 분리됨. ADR-0001 본문 갱신 필요.
- **ADR-0001 컴포넌트 #2** (멀티모달 LLM) 의 *쿼리 시점 멘션 추출* 용도가 제거됨. 컴포넌트 목록 갱신.
- **PRD 의 "쿼리" 섹션** 이 자연어 입력 가정에서 primitives 노출로 재작성.
- **벤치마크 하니스 (ADR-0001 의 (3) 컬럼)** 가 anchor 추출 → primitives 호출 → LLM 답변 생성을 자체 구현. Opentology 본체에 없는 코드.
...
- Neo4j MCP 가 같은 DB 에 attach 됐을 때 *상호 간섭* 없음을 보장 — Opentology 가 사용하는 인덱스명·라벨에 충돌 없는 prefix 권장 (`opt_*` 등).
- 토큰 측정 (ADR-0005 D6) 에서 *Opentology 컬럼* 의 토큰은 *caller 의 두 LLM 호출 + Opentology primitives 호출 시 발생하는 embedding 호출* 의 합. 명세 명확히.
...
- [ADR-0005 — 측정 방법론](./0005-measurement-methodology-accuracy-tokens-latency.md) — Opentology 컬럼의 토큰 합산 규칙.
