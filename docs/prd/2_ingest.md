...
임베딩 유사도 threshold 0.92 는 *경험적 기본값* 이고, 측정 보고서에서 *오병합과 과병합* 사례가 보이면 *측정 직후 조정* . 변경 시 ADR 또는 본 PRD 의 amend 로 기록.
...
- 비텍스트 멀티모달 (오디오와 동영상) — post-MVP.
- 같은 소스의 *부분 재 ingest* (특정 청크만) — 전체 차분 알고리즘으로 충분.
- 권한과 테넌트별 격리 ingestion — ADR-0002 D2 (단일 환경 가정).
