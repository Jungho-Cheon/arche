...
- **잘못 합친 노드를 떼어내지 못합니다.** 그래서 문서가 사용자에게 "판단이 서지 않으면 따로 두라"고 안내하는데, 이건 흩어짐을 늘리는 방향입니다 → [`docs/backlog.md`](./docs/backlog.md) B-1
- **검토형 적재 5개가 MCP 에만 있습니다.** REST 로 문서를 넣는 길은 검토 단계가 없는 `POST /admin/ingest` 뿐입니다 → [`docs/backlog.md`](./docs/backlog.md) B-2
...
docs/prd        제품 요구사항
docs/backlog.md ADR/PRD 로 확정하지 않은 기능 후보
```
