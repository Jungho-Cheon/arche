...
Cohesion: 0.24
Nodes (11): _make_anthropic_provider(), _make_openai_provider(), _mock_openai_response(), LLMProvider.complete 의 멀티모달 입력 unit 테스트 (이슈 #14)., 문자열 user 가 들어오면 messages.content 가 문자열 — 본 PR 전과 동일., 리스트 user 가 들어오면 그대로 messages.content 에 전달., Anthropic 은 이미지 block 을 보지 않는다 — 텍스트 block 만 합쳐 호출., test_anthropic_complete_with_block_list_flattens_text_only() (+3 more)
