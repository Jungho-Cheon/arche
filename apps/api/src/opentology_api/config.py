...
WHY 환경 변수 prefix `OPENTOLOGY_API_*`: `OPENTOLOGY_EVAL_*` (eval 하니스) 와
...
ADR-0001 통제 변수 — 청크 벡터 RAG (eval/) 의 청크 임베딩과 Opentology 노드
...
class Settings(BaseSettings):
    ...
    llm_model: str = Field(
        default=DEFAULT_LLM_MODEL, alias="OPENTOLOGY_API_LLM_MODEL"
    )
    embedding_model: str = Field(
        default=DEFAULT_EMBEDDING_MODEL, alias="OPENTOLOGY_API_EMBEDDING_MODEL"
    )

    # Neo4j
    neo4j_uri: str = Field(default="bolt://localhost:7687", alias="NEO4J_URI")
    neo4j_user: str = Field(default="neo4j", alias="NEO4J_USER")
    neo4j_password: str = Field(default="opentology", alias="NEO4J_PASSWORD")
