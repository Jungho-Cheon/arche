...
_TOOL_GLOSS_KO: dict[str, str] = {
    ...
    "find_entities": (
        "키워드로 출발점이 될 노드를 찾는다. 거의 모든 질의가 여기서 시작한다. "
        "키워드를 빼면 조건에 맞는 노드를 빠짐없이 훑는 열거가 된다."
    ),
    ...
    "graph_health": (
        "잘못 저장된 노드를 한 번에 찾는다. 타입별 전체 개수와 함께 이름이 겹치는 "
        "노드, 두 대상이 한 노드에 들어간 것, 관계가 하나도 없는 노드를 알려 준다."
    ),
