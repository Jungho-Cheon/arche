...
원칙:
1. 본문에 명시적으로 등장하는 정보만 추출. 추론과 확장 금지.
2. 엔티티 이름은 본문 표기 그대로. 정규화는 별칭 필드로.
   (a) 한 엔티티는 *하나의 대상* 만 가리킨다 — *반드시* 지킬 것 (도메인 무관):
       서로 다른 둘 이상을 쉼표, 슬래시, "및", "와/과" 로 이어 한 이름으로 만들지
       않는다. 목록, 표의 한 행, 인용 묶음처럼 여러 대상이 나란히 등장하면 각각을
       별도 엔티티로 뽑고 필요하면 관계로 잇는다.
       나쁜 예: "Wikipedia, Myers-Briggs Type Indicator" (출처와 문서 두 대상),
       "MBTI Manual 3rd edition, CAPT 자료" (인용 둘).
       좋은 예: "Wikipedia" 와 "Myers-Briggs Type Indicator" 두 엔티티.
       단 쉼표가 한 대상의 이름 일부인 경우(예: "Alphabet, Inc.", "Seoul, Korea")는
       그대로 한 엔티티다. 기준은 글자가 아니라 *가리키는 대상이 몇인가* 이다.
       WHY: 둘을 한 이름에 넣으면 어느 쪽 이름으로 검색해도 안 걸리고, 그 노드를
       지나는 경로가 두 대상을 섞어 거짓이 된다.
3. 관계는 능동형 동사구로 ("적용된다", "포함한다", "대체한다" 등).
4. 같은 엔티티가 본문에 여러 번 나오면 한 번만 추출하고 별칭을 모은다.
   (a) 식별자나 기호와 정식 명칭의 동일성 — *반드시* 지킬 것 (도메인 무관):
       같은 대상이 본문에서 *정식 명칭* 과 *식별자/기호/코드* (등록번호, 유전자/
       단백질 ID, 종목코드, 제품과 부품 코드, 약어 등) 두 형태로 등장하고 둘이 같은
       대상을 가리킴이 맥락상 분명하면, *하나의 엔티티* 로 추출하고 모든 표기를
       aliases 에 넣는다. "정식명칭 (ID)" 또는 "ID (정식명칭)" 패턴, 그리고 본문에서
       교차로 쓰이는 약어/기호/등록번호는 강한 동일성 신호다.
       WHY: 이렇게 묶어야 (1) 어느 표기로 검색해도 같은 노드에 닿고, (2) 여러 문장과
       문서에 흩어진 같은 대상이 한 노드로 병합돼 관계 사슬이 끊기지 않는다. 반대로
       명칭과 ID 를 *별개 노드* 로 두면 한쪽으로 들어온 질의가 다른 쪽의 관계를 못
       본다 (multi-hop 사슬 단절의 주원인).
5. 정량 사실과 측정값 보존 (도메인 무관) — *반드시* 지킬 것:
...
6. 표(table) 완전 추출 — *누락 금지* (도메인 무관):
   본문에 표가 있으면 그 행과 열의 값을 *요약하지 말고* 빠짐없이 옮긴다. 표의 각 행이
