...
기본 공급자인 OpenAI 의 SDK(`openai`)는 기본 의존성이라 이미 깔려 있습니다. 그래서 기본 경로(OpenAI 추출 + 임베딩)는 추가 설치가 필요 없습니다.

Anthropic 과 Voyage 의 SDK(`anthropic`, `voyageai`)는 그 공급자를 실제로 고를 때만 불러오도록 미뤄 뒀습니다. 깔려 있지 않은 채로 이 공급자를 고르면 서버가 뜰 때가 아니라 모델을 부르는 순간에 import 오류가 납니다. `anthropic/*` 나 `voyage/*` 로 바꾸기 전에 아래 명령으로 SDK 를 먼저 설치하세요.
