...
공급자별 파이썬 라이브러리(SDK: `openai`, `anthropic`, `voyageai`)는 해당 공급자를 실제로 고를 때만 불러오도록 미뤄 뒀습니다. SDK 가 깔려 있지 않은 채로 새 공급자를 고르면 서버가 뜰 때가 아니라 모델을 부르는 순간에 import 오류가 납니다. 기본값이 아닌 공급자로 바꾸기 전에 아래 명령으로 SDK 를 먼저 설치하세요.
