...
여기 예시는 모두 `curl` 로 REST API 를 직접 부릅니다. 이건 AI 에이전트를 쓰지 않고 손으로 조회 연산을 확인할 때 쓰는 저수준 방법입니다. Arche 를 쓰는 기본 통로는 에이전트가 MCP 로 붙는 것이고, 여기 나오는 여섯 연산을 MCP 도구로도 똑같이 부릅니다. 에이전트로 붙이는 흐름은 [에이전트에 연결하기](/guide/agent-integration)에서 다룹니다.
