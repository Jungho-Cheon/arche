...
| HTTP 로 비동기 적재 | HTTP 로 띄워 두고 진행을 지켜볼 때 | 상태를 따로 조회 | 없음 |
...
API 를 띄워 두고 HTTP 로 넣는 길입니다. REST 는 API 를 HTTP 주소로 부르는 방식으로, 프로그램에서 연동할 때 씁니다. 적재는 시간이 걸리는 작업이라, 요청을 받으면 바로 작업을 만들어 돌려주고 진행 상황은 따로 조회하게 했습니다. 폴더가 크거나 적재를 다른 시스템에서 걸어 두고 진행을 지켜보고 싶을 때 잘 맞습니다.
...
::: warning 적재 기능을 켠 서버가 필요합니다
아래 네 도구는 적재 기능을 켜서 띄운 MCP 서버에서만 나옵니다. 지금 배포된 CLI 명령 `arche mcp serve --stdio` 는 조회 도구 여섯 개만 노출하고 적재 도구는 켜지 않습니다. 그래서 오늘 문서를 그래프에 넣을 때는 위의 CLI 빠른 적재(`arche ingest`)나 HTTP 비동기 적재(`POST /admin/ingest`)를 씁니다.
:::
