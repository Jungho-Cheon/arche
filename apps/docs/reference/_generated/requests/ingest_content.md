...
| `source_id` | `string` | (필수) | 최소 1자 | 출처 라벨 — 파일 경로를 대신해 같은 출처임을 알아보는 값. 같은 값으로 다시 넣으면 바뀐 부분만 갱신 (예: confluence:PAGE-123, URL) |
| `namespace_id` | `string` | `default` | 최소 1자 | 계획이 속한 namespace. 미지정 시 'default' |
| `hints` | `string \| null` | `null` (없으면 키 제외) | 최대 4000자 | 추출 품질을 높이는 선택 입력 — 도메인 용어/약어 풀이, 대상 엔티티 강조 등. max_length 로 프롬프트 예산을 제한한다. |
