...
같은 기계에서 쓴다면 지금까지처럼 stdio 로 연결해도 됩니다. 저장소만 Neo4j 를 보면 되니까요.

팀원이 각자의 컴퓨터에서 연결하려면 HTTP 전송을 씁니다. API 서버가 `/mcp/v1` 에 이미 마운트하고 있어 따로 띄울 게 없습니다.
...
stdio 와 HTTP 는 **같은 도구 12개**를 같은 이름과 같은 스키마로 노출합니다. 에이전트 쪽에서 바뀌는 건 접속 설정뿐입니다. 전송 방식 선택은 [에이전트와 연결하기](/integrate/agent)에서 더 다룹니다.
