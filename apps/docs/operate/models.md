...
두 모델은 서로 독립입니다. 추출만 바꿔도 되고 임베딩만 바꿔도 됩니다. 다만 **임베딩을 바꾸면 색인을 다시 만들고 문서도 다시 넣어야 합니다.** 추출을 바꿀 때는 그럴 필요가 없습니다.
...
Anthropic 은 임베딩 API 가 없어서 Claude 로 추출할 때 임베딩은 OpenAI 나 Voyage 로 씁니다.
...
Claude Code 플러그인의 기본 설정이 이 조합입니다. 임베딩은 따로라 OpenAI 나 Voyage 키가 여전히 필요합니다.
