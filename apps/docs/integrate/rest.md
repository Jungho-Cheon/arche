...
에이전트로 붙이는 기본 통로는 MCP 입니다. REST 는 자체 서비스에 끼워 넣거나, 에이전트 없이 그래프를 다룰 때 씁니다. MCP 쪽은 [에이전트에 붙이기](/integrate/agent)에 있습니다.
...
`invalid_input` 은 `details.errors[]` 에 어느 필드가 왜 걸렸는지 평탄하게 실어 줍니다. 요청을 만드는 코드를 고칠 때 이 값을 보면 됩니다.
