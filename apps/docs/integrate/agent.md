...
에이전트가 Arche 를 부르게 하는 방법입니다. 붙이는 방식은 셋이고, 에이전트와 Arche 가 같은 기계에 있는지에 따라 갈립니다.
...
도구만 붙이는 게 아니라 언제 무엇을 어떤 순서로 부를지의 사용 패턴까지 스킬로 함께 얹습니다. Claude Code 를 쓴다면 이쪽이 가장 손이 적게 갑니다.
...
| `GET /mcp/v1/sse` 와 `POST /mcp/v1/message` | 예전 HTTP+SSE. 두 주소를 짝으로 씁니다 |
...
**클라이언트를 다시 켰는지** 확인하세요. 설정을 고쳐도 재시작 전에는 안 붙습니다.

**`arche` 명령이 잡히는지** 확인하세요. 클라이언트가 띄우는 셸은 터미널과 PATH 가 다를 수 있습니다. 안 잡히면 `uv` 로 부르는 형태나 절대 경로를 씁니다.

```bash
which arche
```

**서버가 조용히 죽지 않았는지** 봅니다. 저장소나 provider 설정이 틀리면 기동하다 멈춥니다. 같은 설정으로 터미널에서 직접 띄워 보면 오류가 보입니다.
...
- 에이전트 없이 부르려면 [REST 로 붙이기](/integrate/rest)
