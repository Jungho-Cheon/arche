...
<p><sub><em>ἀρχή</em> — 만물이 비롯되는 첫 원리. 한 점에서 모든 관계가 펼쳐진다.</sub></p>
...
Arche 는 문서를 미리 **관계 지도(그래프)** 로 바꿔 두고, 에이전트가 그 위에서 작고 값싼 조회만으로 답에 필요한 연결을 따라가게 한다. 도시 전체를 외우게 하거나(full-context) 거리 사진만 잔뜩 보여주는(청크 검색) 대신, **지하철 노선도와 "한 정거장씩 이동" 버튼** 을 쥐여 주는 셈이다.
